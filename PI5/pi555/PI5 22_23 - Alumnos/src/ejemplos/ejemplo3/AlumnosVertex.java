...
public record AlumnosVertex(Integer index, List<Integer> remaining) 
	...
	public List<Integer> actions() {
		...
		if(index < DatosAlumnos.getNumAlumnos()) {
			alternativas = IntStream.range(0, DatosAlumnos.getNumGrupos()).filter(j->DatosAlumnos.getAfinidad(index, j) > 0 && 
					remaining.get(j) >0).boxed().toList();
		}
	...
	/*
	 * El objetivo es la arista voraz. El que arista cojo = que arista me interesa mas
	 * 
	 */
