...
public class DatosEjercicioCafes {
	...
	public record Variedad(int id, Double beneficio, List<Double> mezcla) { // RECORD PARA LAS VARIEDADES DE CAFE
		...
		public static Variedad create(String linea) {
			...
			Double benef = Double.parseDouble(var[0].split("=")[1].replace(";", "").trim());
	...
	public static Integer getMaxKgVariedad(Integer i) {
		...
		return aux.stream().min(Double::compareTo).get().intValue();
	}

	public static Integer getKilosMaximosVariedad(Integer i, List<Double> remaining) {
		List<Double> aux = new ArrayList<>();
		for(int j = 0; j < tipos.size(); j++) {
			if(getPorcentaje(i, j) != 0.0) {
				aux.add(remaining.get(j) / getPorcentaje(i, j));
			}
